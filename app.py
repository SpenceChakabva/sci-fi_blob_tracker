...
# Blob detector params (tuned for better detection)
params = cv2.SimpleBlobDetector_Params()
params.filterByColor = False
params.filterByArea = True
params.minArea = 70    # Increased minArea for cleaner blobs
params.maxArea = 3000
params.filterByCircularity = True
params.minCircularity = 0.4  # Stricter circularity to avoid noise
...
prev_gray = None  # For frame differencing
...
while True:
    ...
    # Sharpen frame for better blob edges
    ...
    _, motion_mask = cv2.threshold(frame_diff, 20, 255, cv2.THRESH_BINARY)  # Raised threshold

    # Morphology to reduce noise (open then close)
    ...
    # Detect blobs on motion mask
    keypoints = blob_detector.detect(motion_mask)

    # Electric cyber glitch effect with subtle RGB shifts on random slices
    for i in range(0, frame.shape[0], 20):
        if random.random() < 0.07:
            shift = random.randint(-15, 15)
            # Split channels
            b, g, r = cv2.split(frame[i:i+10])
            # Shift channels differently for glitch effect
            b = np.roll(b, shift, axis=1)
            g = np.roll(g, -shift, axis=1)
            r = np.roll(r, shift // 2, axis=1)

            # Merge with subtle color tint (cyan-magenta-yellow vibes)
            glitch_slice = cv2.merge([b, g, r])
            # Blend glitch slice softly over original slice for subtlety
    ...
    # Overlay head images on first blob
    ...
    cv2.imshow("Sci-fi Blob Tracker - Electric Cyber Glitch", frame)
